...
// [ISO] EVENT: xxx | DELAY: xms | STATUS: xxx | DETAIL | META: {...}
function logEvent({ event_id, delayMs, status, detail = '', metadata = null }) {
  const metaText = metadata ? ` | META: ${JSON.stringify(metadata)}` : '';
  const line = `[${new Date().toISOString()}] EVENT: ${event_id} | DELAY: ${delayMs ?? '-'}ms | STATUS: ${status}${detail ? ' | ' + detail : ''}${metaText}`;
  console.log(line);
  try {
    ensureLogDir();
