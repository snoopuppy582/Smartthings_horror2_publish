...
describe('safetyFilter - allowlist', () => {
  ...
  it('실험용 player_hit event_id는 통과', () => {
    const plan = { actions: [{ device: 'fan_plug', type: 'setSwitch', value: 'on', duration_ms: 5000 }, { type: 'restore', device: 'all' }] };
    const result = filterPlan('player_hit', plan);
    assert.equal(result.allowed, true);
  });
...
describe('safetyFilter - duration clamp', () => {
  ...
  it('smart_light setLevel duration이 5000ms 초과시 5000ms로 clamp', () => {
    const plan = {
      actions: [
        { device: 'smart_light', type: 'setLevel', value: 50, duration_ms: 12000 },
        { type: 'restore', device: 'all' },
      ],
    };
    const result = filterPlan('ghost_hint', plan);
    const lightAction = result.actions.find(a => a.device === 'smart_light');
    assert.equal(lightAction.duration_ms, 5000);
  });

  it('smart_light setLevel 값이 20 미만이면 20으로 clamp', () => {
    const plan = {
      actions: [
        { device: 'smart_light', type: 'setLevel', value: 5 },
        { type: 'restore', device: 'all' },
      ],
    };
    const result = filterPlan('mission_failed', plan);
    const lightAction = result.actions.find(a => a.device === 'smart_light');
    assert.equal(lightAction.value, 20);
  });
});
...
describe('safetyFilter - strobe 금지', () => {
  it('flash repeat>1 은 repeat=1로 강제', () => {
    ...
    assert.equal(plan.actions[0].repeat, 5);
  });
});
