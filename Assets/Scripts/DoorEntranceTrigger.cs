...
// 문 개구부에 두는 트리거 콜라이더 — 플레이어 통과 시 blackout 발생
...
public class DoorEntranceTrigger : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (ExperimentDirector.Instance != null && ExperimentDirector.Instance.IsRunning)
            ExperimentDirector.Instance.ReportProgress(ExperimentProgressMarker.MarkerType.Custom, "blackout");
        else if (SmartThingsEventSender.Instance != null)
            SmartThingsEventSender.Instance.SendEvent("blackout");
        else if (STInteracter.Instance != null)
            STInteracter.Instance.SendEvent("blackout");
        else
            Debug.LogWarning("[DoorEntrance] SmartThings sender 없음 — blackout 전송 생략");

        Debug.Log("[DoorEntrance] 플레이어 진입 — blackout 전송");
    }
}
